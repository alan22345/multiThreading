package InterThreadComms.Matrices;
...
public class MainApplication {
    ...
    private static final String OUTPUT_FILE = "./src/InterThreadComms/Matrices/matrices_results.txt";
    ...
    private static class ThreadSafeQueue {
        ...
        private static final int CAPACITY = 5; // here we apply back pressure to avoid out of memory exception

        public synchronized void add(MatricesPair matricesPair) { // called by producer add pair matrices to queue
            while (queue.size() == CAPACITY) { //check if capacity of queue has been reached, wait until woken up
                try {
                    wait();
        ...
        public synchronized MatricesPair remove() { //called by consumer to remove matrices
        ...
        public synchronized void terminate() { // called by producer to let consumer know that once
                                               // queue is empty consumer should terminate thread
            isTerminate = true;
            notifyAll();
        }
        // all methods are synced to keep operations atomic and give us access to the wait, notify etc methods
    }
